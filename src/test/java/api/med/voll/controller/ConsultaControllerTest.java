...
import api.med.voll.domain.consulta.DadosDetalhamentoConsulta;
...
class ConsultaControllerTest {
	...
	@Test
	@DisplayName("Deverá retornar codigo http 400 quando informações inválidas")
	@WithMockUser
	...
	@Test
	@DisplayName("Deverá retornar codigo http 200 quando informações válidas")
	@WithMockUser
	void agendarCenario02() throws Exception {
		...
		var response = mvc.perform(
				...
				.content(jsonRequest.write(new DadosAgendamentoConsulta(1l, 5l, data, especialidade)).getJson())
				)
		.andReturn().getResponse();
		
		assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
		
		var jsonReturn = jsonResponse.write(dadosDetalhamento).getJson();
